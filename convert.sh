...
echo "resource \"aws_security_group\" \"tier1apps\" {" >> securitygroups.tf
echo "			name        = \"tier1apps-sg\"" >> securitygroups.tf
echo "			description = \"Security group for 3-tier apps\"" >> securitygroups.tf
echo "			vpc_id      = \"\${aws_vpc.3tier.id}\"" >> securitygroups.tf
while read -r line
do
    cidrblock="$line"
...
# Add local ip as rule to tier 1 security group
...
echo "resource \"aws_security_group\" \"tier2apps\" {" >> securitygroups.tf
echo "                  name        = \"tier2apps-sg\"" >> securitygroups.tf
echo "                  description = \"Security group for 3-tier apps\"" >> securitygroups.tf
echo "                  vpc_id      = \"\${aws_vpc.3tier.id}\"" >> securitygroups.tf
